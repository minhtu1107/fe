...
const config = {
  ...
  UseHTTPS:true,
  UseMatchmaker:true,
  publicIp: '3.113.18.81',
  peerConnectionOptions: '{"offerExtmapAllowMixed":false , "iceServers":[{"urls":["stun:stun.l.google.com:19302","turn:3.113.18.81:19303"],"username":"PixelStreamingUser","credential":"Another TURN in the road"}]}'
};

const port = process.env.PORT || config.UseHTTPS?80:3000;
const httpsPort = 443;
const certPath = 'C:\\test';
...
// app.prepare().then(() => {
  ...
  let playerServer = new WebSocket.Server({ server: config.UseHTTPS ? https : http, path:'/streamming'});
  if(!config.debug) {
 ...
 */
 ...
 if (config.UseMatchmaker) {
    ...
    var matchmakerAddress = '54.249.83.187';
	...
	matchmaker.on('connect', function() {
		...
		let addr = typeof serverPublicIp === 'undefined' ? '127.0.0.1' : serverPublicIp;
		message = {
			type: 'connect',
			address: addr,
			port: config.UseHTTPS ? httpsPort : port,
			socketUrl: 'wss://' + addr + ':' + (config.UseHTTPS ? httpsPort : port) + '/streamming'
		};
